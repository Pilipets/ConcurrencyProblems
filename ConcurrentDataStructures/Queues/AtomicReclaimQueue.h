#pragma once

#include <optional>
#include "QeueuNode.h"
#include "../DeleteStrategies/ReclaimBase.h"

namespace concurrent::ds::queues {

	template <class T>
	class AtomicReclaimQueue : memory::ReclaimBase<AtomicRawNode<T>>{
		using Node = AtomicRawNode<T>;
		using NodePtr = Node::NodePtr;
		using Base = memory::ReclaimBase<AtomicRawNode<T>>;

		AtomicReclaimQueue(AtomicReclaimQueue&) = delete;
		AtomicReclaimQueue& operator=(AtomicReclaimQueue&) = delete;

		std::atomic<NodePtr> head, tail;

	public:
		AtomicReclaimQueue() : head(new Node()), tail(head.load()) {}
		~AtomicReclaimQueue() { while (pop()); delete head; }

		void push(T val) {
			NodePtr new_node = new Node(std::move(val));

			while (true) {
				NodePtr old_tail = tail.load(), dummy{};

				if (old_tail->next.compare_exchange_weak(dummy, new_node)) {
					tail.compare_exchange_weak(old_tail, new_node);
					return;
				}
				else {
					tail.compare_exchange_weak(old_tail, old_tail->next.load());
				}
			}
		}

		std::optional<T> pop() {
			++Base::threads_in_pop;

			while (true) {
				NodePtr old_head = head, old_tail = tail;
				NodePtr next_head = old_head->next;

				if (old_head == old_tail) {
					if (next_head == nullptr) return std::nullopt;
					else tail.compare_exchange_weak(old_tail, next_head);
				}
				else {
					if (head.compare_exchange_weak(old_head, next_head)) {
						T res = std::move(next_head->val);
						Base::try_reclaim(old_head);
						return res;
					}
				}
			}
		}

		bool empty() const { return head.load()->next.load() == nullptr; }
	};

}